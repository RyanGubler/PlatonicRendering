...
MySample.graphics = (function(pixelsX, pixelsY, showPixels) {
    ...
    //
    //------------------------------------------------------------------
    function drawPrimitive(primitive, connect, centerPoint, color) {
        let {verts} = primitive;
    ...
    function translateCurve(type, controls, distance) {
        if(type === "Bezier" || type === "BezierMatrix"){
            for(let i = 0; i < controls.length; i++){
                controls[i][0] = controls[i][0] + distance.x;
                controls[i][1] = controls[i][1] + distance.y;
            }
    ...
    function rotateCurve(type, controls, angle) {
        ...
        if(type === "Bezier" || type === "BezierMatrix"){
            for(let i = 0; i < controls.length; i++){
                let translateOrigin = translatePoint({x: controls[i][0], y: controls[i][1]}, {x: -centerX, y: -centerY});
                let rotate1 = (translateOrigin.x * Math.cos(angle)) - (translateOrigin.y * Math.sin(angle));
                let rotate2 = (translateOrigin.x * Math.sin(angle)) + (translateOrigin.y * Math.cos(angle));
                controls[i][0] = rotate1
                controls[i][1] = rotate2
                let translate = translatePoint({x: controls[i][0], y: controls[i][1]}, {x: centerX, y: centerY});
                controls[i][0] = translate.x;
                controls[i][1] = translate.y;
             }
